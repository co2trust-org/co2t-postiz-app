/** Shared Brand Brain shapes + prompt formatting — used by API, agent, and frontend. */
...
export function formatBrandBrainForPrompt(data: BrandBrainPersisted): string {
  ...
  if (data.links.length) {
    ...
    for (const l of data.links) {
      ...
      if (a && b) {
        lines.push(`  • "${a.label}" -> "${b.label}"${rel}`);
      }
    }
  }
  lines.push(
...
function isBrandBrainPersisted(raw: unknown): raw is BrandBrainPersisted {
  ...
  return (
    o.version === 1 &&
    Array.isArray(o.brands) &&
    Array.isArray(o.concepts) &&
    Array.isArray(o.links)
  );
}

/** Coerce persisted JSON into a valid BrandBrainPersisted or default. */
export function parseBrandBrainFromJson(raw: unknown): BrandBrainPersisted {
  return isBrandBrainPersisted(raw) ? raw : createInitialBrandBrain();
}
